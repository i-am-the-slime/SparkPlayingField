...
//  "RDDs of Events" should "be serializable to disk in binary format" in {
  ...
  it should "please work" in {
    ...
    //val file = File("./shitfucks.hit")
    //dataFileWriter.create( A., file)


    val record = A(1)

    val file = java.io.File.createTempFile("AvroRecordTest48", "avro")
    file.deleteOnExit()
    ...
    val userDatumReader = new SpecificDatumReader[A](schema)
    val dataFileReader = new DataFileReader[A](file, userDatumReader)
    val sameRecord = dataFileReader.next()

    sameRecord should ===(record)


    //Reading avros - no reflection allowed since Scala fields are always private
    //val schema = new DataFileReader(, new GenericDatumReader[GenericRecord]).getSchema
